...
Locator: locator('h2').filter({ hasText: 'Добавьте канал' })
Expected: visible
Error: element(s) not found

Call log:
  - Expect "toBeVisible" with timeout 5000ms
  - waiting for locator('h2').filter({ hasText: 'Добавьте канал' })
...
```ts
  ...
  66 |     await accountPage.open();
  67 |     await accountPage.openChannels();
  68 |     await channelPage.deleteChannel();
  69 |     await accountPage.open();
  70 |     await accountPage.openChannels();
  71 |     
  72 |     await page.reload();
> 73 |     expect(page.locator('h2').filter({ hasText: 'Добавьте канал' })).toBeVisible();
     |                                                                      ^ Error: expect(locator).toBeVisible() failed
  74 | });
```