...
> 34 |     expect(getUserApi.body.id).toBe(userApi.body.id);
     |                                                  ^ TypeError: Cannot read properties of undefined (reading 'id')
  35 |     expect(getUserApi.body.name).toBe(userApi.body.name);
  36 |     expect(getUserApi.body.phone).toBe(userApi.body.phone);
  37 | 
  38 | });
```