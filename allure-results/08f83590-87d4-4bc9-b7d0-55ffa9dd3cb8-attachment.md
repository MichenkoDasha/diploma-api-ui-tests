...
Received string:    "Вы добавили канал. На нем автоматически включился автоответ: «Первое входящее»"

Call log:
  - Expect "toContainText" with timeout 5000ms
  - waiting for getByRole('status')
    2 × locator resolved to <div role="status" aria-live="polite" class="v-snackbar__content">…</div>
      - unexpected value "Вы добавили канал. На нем автоматически включился автоответ: «Первое входящее»"
...
```ts
  ...
  67 |    
> 68 |     expect(page.getByRole('status')).toContainText('Канал удален из сервиса');
     |                                      ^ Error: expect(locator).toContainText(expected) failed
  69 | });
```